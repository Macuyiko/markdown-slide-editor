...
my_env = os.environ
my_env["PATH"] = (
    "C:\\Program Files (x86)\\Graphviz2.38\\bin\\" + os.pathsep + my_env["PATH"]
)
...
extension_configs = {
    "mdx_math": {"enable_dollar_delimiter": False, "add_preview": True}
}
...
def postprocess_pages(html, with_pages="allbutfirst", with_footer=""):
    ...
    for p, page in enumerate(pages):
        ...
        if (with_pages == "allbutfirst" and page_num == 1) or with_pages == "none":
            ...
            page_num_data = "true"
        output += (
            '<div class="slide-wrapper" id="page_{}" data-page-number="{}">'.format(
                page_num, page_num_data
            )
        )
        output += (
            '<div class="slide"><div class="slide-bg"></div><div class="slide-inner">'
        )
        output += page
        output += "</div>"
        output += '<footer class="slide-footer">{}</footer>'.format(with_footer)
        output += (
            '<span class="slide-page" data-page="{}">{}</span></div></div>'.format(
...
def postprocess_backgrounds(html):
    ...
    for background_image in h.select('p > img[alt~="bg"]'):
        ...
        parent_wrapper_bg = background_image.find_parent(class_="slide-wrapper").find(
            class_="slide-bg"
        )
        ...
        new_div = h.new_tag(
            "div", **{"class": "slide-bg-img", "style": new_style, "data-alt": alt}
        )
        parent_wrapper_bg.append(new_div)
        background_image.decompose()
        if not parent_p.find(lambda x: x.name != "br") and not parent_p.get_text(
            strip=True
        ):
            parent_p.decompose()
    return str(h)
...
def postprocess_images(html):
    ...
    for image in h.select('img[alt*="%"]'):
        ...
        for alt_piece in re.compile(r"\s+").split(alt):
            percentage = extract_percentage(alt_piece)
            if percentage:
                image["style"] = image.get("style", "") + "; zoom: {};".format(
                    float(percentage) / 100.0
                )
            percentage = extract_percentage(alt_piece, opacity_regex)
            if percentage:
                image["style"] = image.get("style", "") + "; opacity: {};".format(
                    float(percentage) / 100.0
                )
    return str(h)
...
def postprocess_only_elements(html):
    ...
    for slide_wrapper in h.select(".slide-wrapper"):
        ...
        if len(innerQuotes) > 0 and len(innerContents) == len(innerQuotes):
            slide_wrapper["class"] = slide_wrapper.get("class", []) + [
                "only-blockquotes"
            ]
    return str(h)
